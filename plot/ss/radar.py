...
def result_pic(result, index):
    ...
    labels = [ 'user','carts','order','shipping','payment','front-end','catalogue']
    new_labels = [ 'user','carts','order','shipping','payment','front-end','catalogue','user']
    
    kinds = list(result.iloc[:, 0])


    # 由于在雷达图中，要保证数据闭合，这里就再添加L列，并转换为 np.ndarray
    result = pd.concat([result, result[['user']]], axis=1)
